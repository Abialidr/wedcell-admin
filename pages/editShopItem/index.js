...
import { AlternateEmail, Height } from '@mui/icons-material';
...
function AddShopItems() {
  ...
  const [status, setStatus] = useState(router.query.id ? 'Edit' : 'Submit');
      ...
      await axios
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            console.log(
              '🚀 ~ file: index.js:425 ~ .then ~ res.data.data.product:',
              res.data.data.product
            );
            setresdata123(res.data.data.product);
  ...
  const deleteVariant = async (data, key) => {
    console.log(`🚀 ~ file: AddShopItems.js:453 ~ deleteVariant ~ data:`, data);
    if (data._id) {
      try {
  ...
  const updateVariant = async (data, type, key) => {
    ...
    fileListmain[key].images
      ...
      .forEach((images) => formdata.append(`album`, images.originFileObj));
    fileListmain[key].main
      .filter((data) => data.originFileObj)
      .forEach((main) => formdata.append(`main`, main.originFileObj));
  ...
  const addHandler = () => {
    ...
    if (
      ...
      !form.contactEmail.length ||
      !form.contactPhone.length ||
    ...
    fileListmain.map((data, key) => {
      data.images
        .filter((data) => data.originFileObj)
        .forEach((images) =>
          formdata.append(`album${key}`, images.originFileObj)
        );
      data.main
        .filter((data) => data.originFileObj)
        .forEach((main) => formdata.append(`main${key}`, main.originFileObj));
      data.videos
        .filter((data) => data.originFileObj)
        .forEach((videos) =>
  ...
  return (
    <div className='bg-white py-2'>
      <h1 className='text-center'>{id ? 'Edit' : 'Add'} Shop Items</h1>
      {isLoading || !form ? (
        'Hello'
      ) : (
